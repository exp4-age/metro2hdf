...
pub fn parseChannel(dir: std.Io.Dir, run: Run, h5f: *H5File, io: std.Io, allocator: std.mem.Allocator) void {
    switch (run.format) {
        ...
        .tdc => {
            parseHptdcChannel(dir, run, h5f, io, allocator) catch |err| {
                std.log.info("  skipping: {s}", .{@errorName(err)});
            };
        },
        else => {
...
pub fn parseHptdcChannel(dir: std.Io.Dir, run: Run, h5f: *H5File, io: std.Io, allocator: std.mem.Allocator) !void {
