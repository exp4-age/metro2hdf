...
pub fn parseChannel(
    ...
    switch (run.format) {
        .txt => {
            tsv.parseChannel(run, file, h5f, io, allocator) catch |err| {
                std.log.info("  skipping: {s}", .{@errorName(err)});
            };
        },
        .tdc => {
            hptdc.parseChannel(run, file, h5f, io, allocator) catch |err| {
                std.log.info("  skipping: {s}", .{@errorName(err)});
            };
        },
        else => {
            std.log.info("  skipping: {s}", .{@errorName(error.UnknownFormat)});
        },
    }
}
